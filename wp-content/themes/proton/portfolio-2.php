...
<?php get_header(); ?>
	...
	<div class="portfolio portfolio_2_container">
	<?php $loop = null;
	...
	?>	
		<article id="post-<?php the_ID(); ?>" class="post_content grid_6">
			...
			?>
				<div class="post_thumbnail <?php if ($theme_post_featured_image_media == "1") {echo "media";} ?>">
					<a href="<?php echo $url.'?width='.(int)$theme_post_featured_image_media_width.'&height='.(int)$theme_post_featured_image_media_height; ?>" rel="prettyPhoto" title="<?php the_title(); ?>" ><img src="<?php bloginfo('template_directory'); ?>/thumb.php?src=<?php echo $image_src[0]; ?>&w=440&h=200" /></a>
				</div>
			<?php } ?>			
			<?php if ($theme_post_portfolio_teaser) { echo '<p>'.$theme_post_portfolio_teaser.'</p>'; } else { the_excerpt(); }?>
		</article>
		<?php if (($i%2)==0) { ?> <div class="clear"></div><br /> <?php } ?>
	<?php endwhile; ?>

...
<?php if ($comments) : // there are comments ?>
		...
		<section class="comments-template">
			...
			<article <?php echo $oddcomment; ?>id="comment-<?php comment_ID(); ?>">
				<header>
					<h4><?php echo get_avatar( $comment, 32 ); ?> <cite><?php comment_author_link(); ?></cite></h4>
				...
				<section>
					<?php comment_text(); ?>
					<p class="comment-meta"><?php comment_date('F jS, Y'); ?> at <?php comment_time(); ?> <?php edit_comment_link('Edit',' &bull; ',''); ?></p>
				</section>
			</article>
